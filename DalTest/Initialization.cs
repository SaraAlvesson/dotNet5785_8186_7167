...
public static class Initialization
{
        ...
        var addresses = new List<Tuple<string, double, double>>()
    {
        ...
        // יצירת מערך של 15 תעודות זהות אקראיות
        int[] ids = new int[]
        {
        328958186, 291827364, 482756019, 572830194, 618273945,
        184729303, 472930167, 348127946, 905374819, 573820195,
        ...
        for (int i = 0; i < volunteerCount; i++)
        {
            ...
            DistanceType distanceType = DistanceType.AerialDistance; // ברירת מחדל
            int maxDistance = s_rand.Next(5, 20); // נניח שיש לנו מרחק רנדומלי
            ...
            s_dal.Volunteer.Create(new Volunteer
            {
                ...
                Active = s_rand.Next(0, 2) == 1, // רנדומליות במצב פעילות
        ...
        var callAddresses = new List<Tuple<string, double, double>>()
    {
        ...
        for (int i = 0; i < 50; i++)
        {
            ...
            switch (callType)
            {
                case CallType.PreparingFood:
                    description = "Cooking or assembling meals for those in need.";
                    break;
                case CallType.TransportingFood:
                    description = "Delivering prepared food to designated locations.";
                    break;
                case CallType.FixingEquipment:
                    description = "Repairing essential tools or equipment.";
                    break;
                case CallType.ProvidingShelter:
                    description = "Arranging or offering temporary accommodation.";
                    break;
                case CallType.TransportAssistance:
                    description = "Helping with vehicle issues or emergency rides.";
                    break;
                case CallType.MedicalAssistance:
                    description = "Delivering medical supplies or offering first aid.";
                    break;
                case CallType.EmotionalSupport:
                    description = "Providing mental health support through conversations.";
                    break;
                case CallType.PackingSupplies:
                    description = "Organizing and packing necessary supplies for distribution.";
                    break;
            }
            ...
            // Select a random call address from Jerusalem
            var selectedAddress = callAddresses[s_rand.Next(callAddresses.Count)];

            // הגדרת זמני פתיחה וסיום
            // זמן פתיחה בטווח הגיוני (לפני הזמן הנוכחי) בהתבסס על השעון המערכת
            DateTime openTime = s_dal.config.Clock.AddHours(-s_rand.Next(1, 6));

            // הגדרת זמן סיום אקראי או null
            DateTime? maxTime = (s_rand.NextDouble() > 0.3)
                ? (DateTime?)openTime.AddHours(s_rand.Next(2, 5)) // אם לא null, הגדרת זמן סיום בטווח הגיוני
                : null; // או משאיר null במקרה שאין זמן סיום

            // הגדרת קריאות שפג תוקפן (במקרה של זמן סיום שהיה בעבר)
            if (maxTime == null && s_rand.NextDouble() < 0.1)
            {
                maxTime = openTime.AddHours(-s_rand.Next(6, 24)); // קריאה שפג תוקפן, זמן סיום לפני הזמן הנוכחי
            }
            // Create the call
            Call newCall = new Call
            {
                Id = callId,
                VerbDesc = description,  // Set the description based on the call type
            ...
            // Insert the new call into the database (DAL)
